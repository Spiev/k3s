...
|---|---|---|---|
| `photos.<your-domain>` | Immich | pi1:2283 (Docker) | pi1:2283 (temp external route) |
| `paperless.<your-domain>` | Paperless | pi2:80 (k3s) | pi2 (k3s, direct) |
| `rss.<your-domain>` | FreshRSS | pi2:80 (k3s) | pi2 (k3s, direct) |
| `ha.<your-domain>` | Home Assistant | pi1:8123 (Docker) | pi1:8123 (temp external route) |
| `files.<your-domain>` | Seafile | pi2:80 (k3s) | pi2 (k3s, direct) |
...
kubectl describe certificate homelab-tls -n kube-system
...
```bash
kubectl delete secret homelab-tls -n kube-system
kubectl apply -f infrastructure/cert-manager/certificates.yaml
```
...
spec:
  redirectRegex:
    regex: "^https://paperless\\.<your-domain>.de/admin/login.*"
    replacement: "https://paperless.<your-domain>/"
    permanent: false
```
...
spec:
  ...
  routes:
    ...
    - match: Host(`photos.<your-domain>`) && PathPrefix(`/api/auth`, `/api/user`)
      kind: Rule
      priority: 10
      middlewares:
        - name: security-headers
...
spec:
  ...
  routes:
    ...
    # Strict rate limit for auth endpoints
    - match: Host(`ha.<your-domain>`) && PathPrefix(`/auth/login`, `/auth/token`)
      kind: Rule
      priority: 10
      middlewares:
        - name: security-headers
...
echo "<K3S_METALLB_VIP> photos.<your-domain>" | sudo tee -a /etc/hosts
curl -v https://photos.<your-domain>/api/server/ping
# Remove after test: sudo sed -i '/photos.<your-domain>/d' /etc/hosts
```
...
curl -v https://photos.<your-domain>/api/server/ping
curl -v https://ha.<your-domain>
curl -v https://rss.<your-domain>
curl -v https://paperless.<your-domain>
curl -v https://files.<your-domain>

# Check cert is from cert-manager (not the old certbot cert)
echo | openssl s_client -connect photos.<your-domain>:443 2>/dev/null | openssl x509 -noout -issuer -dates
```
...
```bash
curl -I https://photos.<your-domain> | grep -iE "strict-transport|x-content|x-frame|referrer|permissions"
```
