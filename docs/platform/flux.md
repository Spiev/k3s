...
└── apps.yaml                    ← points Flux at apps/, enables SOPS decryption + cluster-vars substitution
```

---

## cluster-vars: variable substitution for app manifests

Some values are the same across multiple services but must not be hardcoded in manifests (e.g. internal IPs, hostnames). These live in `infrastructure/cluster-vars/cluster-vars.sops.yaml` — a SOPS-encrypted Secret — and are injected into manifests at reconcile time via Flux's `postBuild` substitution.

### How it works

```
infrastructure/cluster-vars/cluster-vars.sops.yaml
  → Flux decrypts (SOPS) → cluster-vars Secret in flux-system namespace
        │
        │  postBuild.substituteFrom in apps.yaml
        ▼
  apps/**/*.yaml: ${VAR} placeholders replaced before kubectl apply
```

The current `apps.yaml` already has this configured:
...
1. Open the encrypted file with SOPS (YubiKey must be plugged in):

```bash
sudo systemctl start pcscd
sops infrastructure/cluster-vars/cluster-vars.sops.yaml
```

2. Add the new key under `stringData`:

```yaml
stringData:
  K3S_IP: "..."          # already there
  MQTT_HOST: "..."       # example — add new vars here
```
...
```bash
git add infrastructure/cluster-vars/cluster-vars.sops.yaml
git commit -m "feat(cluster-vars): add <VAR_NAME>"
git push
```

5. Reference in any manifest under `apps/`:
...
Flux substitutes the value before applying. The placeholder is never sent to the cluster.
